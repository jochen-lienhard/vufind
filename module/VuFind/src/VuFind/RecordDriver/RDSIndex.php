...
class RDSIndex extends SolrMarc
{
    ...
    */
    ...
    /**
     * Get the Coins based on OpenURL parameters to represent this record 
     *
     * @return string OpenURL parameters.
     */
    public function getCoinsOpenUrl() 
    {
        ...
        $params['rft.place']   = $this->getPublisher();
        ...
        $pages = $this->getPhysicalDescriptions();
        if (isset($pages)) {
            $params['rft.tpages']=$this->getPhysicalDescriptions();
        }
        ...
        foreach ($params as $key => $value) {
            if (is_array($value)) {
                $parts[] = $key . '=' . urlencode(implode(';', $value));
            } else {
    ...
    {
        if (($isbn = $this->getCleanISBN()) && isset($this->mainConfig->Content->coverimages)) {
    ...
    {
        $arr = array();
        if (isset($this->fields['ti_long_f'])) {
            $arr_links = $this->fields['ti_long_f'];
            if (strstr($arr_links, "|")) {
                $arrlink = explode(" | ", $arr_links);
                $arr[$key]['txt'] = $arrlink[0];
                $arr[$key]['id'] = $arrlink[1];
            } else {
                $arr[$key]['txt'] = $arr_links;
            }
        }
        return $arr;
    ...
    /**
     * Get an array of all other titles.
     * RDS
     * @return array
     */
    public function getOtherTit() 
    {
        return isset($this->fields['other_tit']) ? $this->fields['other_tit'] : '';
    }
            ...
            foreach ($arr_links as $key => $link) {
                ...
                if (strstr($link, "|")) {
                    ...
                    if (substr_count($zdb_nr, $arr_link[0])> 0) {
                        if (isset($arr_link[3]) && $arr_link[3] != "") {
                            $zs_array[$key]['pre-text'] = $arr_link[1];
                            $zs_array[$key]['id'] = $arr_link[0];
                            $zs_array[$key]['text'] = $arr_link[2];
                            $zs_array[$key]['zusatz'] = $arr_link[3];
                        }
                        if ($arr_link[2] != "" && !isset($arr_link[3])) {
                            $zs_array[$key]['pre-text'] = $arr_link[1];
                            $zs_array[$key]['id'] = $arr_link[0];
                            $zs_array[$key]['text'] = $arr_link[2];
                        }
                        ...
                        if ((isset($arr_link[2]) && $arr_link[2] != "") && $arr_link[1] != "") {
                            if (!preg_match("/^[0-9]{8,}/", $arr_link[0])) {
                                $zs_array[$key]['pre-text'] = $arr_link[0];
                                $zs_array[$key]['text'] = $arr_link[1];
                                $zs_array[$key]['zusatz'] = $arr_link[2];
                            } else {
                                $zs_array[$key]['pre-text'] = $arr_link[1];
                                $zs_array[$key]['text'] = $arr_link[2];
                            }
    ...
    /**
     * Get a string of zdb numbers for journal online and print.
     * RDS
     * @return string
     */
    ...
    public function supportsAjaxStatus()
    {
        if ($this->getONLINE() == "offline") {
            if (in_array('zeitschrift', $this->getFormats())) {
                return false;
            } else {
                return true;
            }
    ...
    public function getPersistentLink() 
    {
        ...
        $persistentLinkUrl = 'https://' . $_SERVER['HTTP_HOST'] . $basePath . '/link?id=' . $id; 
