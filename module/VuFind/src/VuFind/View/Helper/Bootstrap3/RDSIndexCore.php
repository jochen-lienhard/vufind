...
class RDSIndexCore extends \Zend\View\Helper\AbstractHelper implements TranslatorAwareInterface
{
    ...
    */
    protected function getCOLLTITLE()
    {
        ...
        if (isset($titleLong_f) && !empty($titleLong_f)) {
            foreach ($titleLong_f as $field) {
                $last_item = end($titleLong_f);
                if (!empty($field['id'])) {
                    $html_result .= "<a class='link-internal' href="
                    .$this->baseUrl.$this->view->render('/RecordDriver/RDSIndex/link-id.phtml', ['lookfor' => $field['id']]).">"
                    .$transEsc($field['txt'])."</a>";
                } else {
                    if (!empty($field['txt'])) {
                        $html_result .=  $transEsc($field['txt']); 
                    }
                }
                if ($titleLong_f != $last_item) {
                    $html_result .="<br /> " ; 
                }
            }
        }
        ...
        if (isset($authors_long) && !empty($authors_long)) {
            foreach ($authors_long as $field) {
                ...
                if (isset($field['gnd']) && !empty($field['gnd'])) {
                    $html_result .= " <a class='dnb link-external' href="
                    ...
                    $html_result .= $this->view->render('/RecordDriver/RDSIndex/link-wiki.phtml', ['lookfor' => $mywiki]);
                }
                if ($field != $last_item) {
                    $html_result .="<br />" ; 
                }
    ...
    protected function getCORP()
    {
        ...
        if (isset($co_long) && !empty($co_long)) {
            foreach ($co_long as $field) {
                ...
                $html_result .= "<a class='link-internal' href=".$this->baseUrl.$this->view->render('/RecordDriver/RDSIndex/link-corporate.phtml', ['lookfor' => $field['link']]).">"
    ...
    */
    ...
    {
        $html_result = "";
        if (preg_match("/^[A-Z].*/", $this->getPpn())) {
            $transEsc = $this->getView()->plugin('escapeHtml');
            $result = $this->driver->getEbookLink();
            if (isset($result) && !empty($result)) {
                foreach ($result as $field) {
                    $last_item = end($result);
                    $html_result .= "<a class='link-external' href='".$field['url']."'>".$transEsc($field['lnk_txt'])."</a>";
                    if ($result!= $last_item ) {
                        $html_result .="<br /> " ; 
                    }
                }
    ...
    protected function getFN() 
    {
        ...
        if (!empty($result)) {
            foreach ($result as $field) {
                $last_item = end($result);
                if (!empty($field['url'])) {
                    $html_result .=  $transEsc($field['text'])."<a class='link-external' href=".$field['url'].">".$field['url']."</a>";
                } else {
    ...
    */
     ...
     * @return html_result 
    */
    protected function getOTHERTIT() 
    {
        $html_result = "";
        $transEsc = $this->getView()->plugin('escapeHtml');
    ...
    protected function getBIBLZUS() 
    {
        ...
        if (isset($zs_info) && !empty($zs_info)) {
            foreach ($zs_info as $field) {
                ...
                if (!empty($field['id'])) {
                    $html_result .= "<a class='link-internal' href=".$this->baseUrl.$this->view->render('/RecordDriver/RDSIndex/link-id.phtml', ['lookfor' => $field['id']]).">"
                    .$transEsc($field['text'])."</a>";
                } else {
                    if (!empty($field['text'])) {
                        $html_result .=  $transEsc($field['text']); 
                ...
                if (!empty($field['zusatz'])) {
                    $html_result .= " ".$transEsc($field['zusatz']);
                }
    ...
    */
     ...
     * @return html_result
     */
    protected function getPERSISTENTLINK() 
    {
        $html_result = "";
        $link = $this->driver->getPersistentLink();
        $html_result .= '<a class="link-internal" href="' . $link . '">' . $link . '</a>';
