      ...
      if (!empty($fulltextLinks)) {
        if ($fulltextLinks[0]['indicator'] == 1) {
          if ($this->driver->getGuestView() != 'brief' && $this->authManager->isLoggedIn() == true) {
            ...
            $html .= '<a class="link-internal getFull" href="' . $this->path . 'RDSProxyrecord/' . $this->uniqueId . '" data-view="'. $dataview .'" onclick="userAction(\'click\', \'RdsFulltextAvailable\', \'' . $this->uniqueId . '\');">'.$this->translate("RDS_FULLTEXT_AVAILABLE").'</a>';
          }
        ...
        $html .= '<a class="link-internal getFull" href="' . $this->path . 'RDSProxyRecord/' . $this->uniqueId . '" data-view="'. $dataview .'" onclick="userAction(\'click\', \'RdsCheckAvailability\', \'' . $this->uniqueId . '\');">'.$this->translate("RDS_CHECK_AVIALABILITY").'</a>';
